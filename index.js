...
function processSum(numberList, callback) {
  const arrSum = numberList.reduce((a,b) => a + b, 0)
...
function getFullNames(runners) {
  let fullNames = []
  runners.forEach((elements) => {fullNames.push(elements.last_name + ', ' + elements.first_name)})
  return fullNames
}
/**
...
function firstNamesAllCaps(runners) {
  //let fullNamesUpper = []
  //runners.forEach((elements) => {fullNamesUpper.push(elements.first_name.toUpperCase())})
  const fullNamesUpper = runners.map((elements) => elements.first_name.toUpperCase())
  return fullNamesUpper
}
...
function getRunnersByTShirtSize(runners,tShirtSize) {
  return runners.filter((elements) => elements.shirt_size === tShirtSize)
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulator, currentValue) => accumulator + currentValue.donation, 0) 
}
/////////////// CLOSURES ///////////////
/////////////// CLOSURES ///////////////
 ...
 * etc
*/
function counterMakerWithLimit(timeLimit) {
  let count = 0;
  return function counter() {
    for (count=0; count < timeLimit; count++) {
      if (count = timeLimit) {
        count = 0;
      } else {
        count++;
